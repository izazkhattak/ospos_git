...
class Sale extends CI_Model
{
	...
	public function get_parent_data($data)
	{
	    $sale_ids = [];
	    foreach($data as $d)
	    {
	    ...
	    $html = '<table class="table table-responsive table-bordered"><thead><tr><th>Name</th><th>Qty</th><th>Price</th></tr></thead><tbody>';
	    foreach($items as $i)
	    {
	        if(!in_array($i['item_id'], $added))
	        {
    	        $this->db->select('sum(quantity_purchased) as qty, sum(quantity_purchased*item_unit_price) as price');
    	        $this->db->from('sales_items');
    	        $this->db->where('item_id', $i['item_id']);
    	        $this->db->where_in('sale_id', $sale_ids);
    	        $inner_data = $this->db->get()->result_array();
    	        foreach($inner_data as $id)
    	        {
    	            if($id['qty'] == 0)
    	                continue;
    	            $parent_data[$count]['name'] = $i['name'];
    	            $parent_data[$count]['qty'] = $id['qty'];
    	            $parent_data[$count]['price'] = $id['price'];
    	            $added[] = $i['item_id'];
    	            
    	            $sum += $id['price'];
    	            $qtys += $id['qty'];
    	            $html .= '<tr><td>'.$parent_data[$count]['name'].'</td><td>'.$parent_data[$count]['qty'].'</td><td>'.$parent_data[$count]['price'].'</td></tr>';
    	            $count++;
    	        }
	        }
	    }
	    $html .= '</tbody></table><h4 class="text-success">Total Amount: <b>'.$sum.'</b></h4><h4 class="text-success">Total Qtys: <b>'.$qtys.'</b></h4>';
	    return $html;
	}
