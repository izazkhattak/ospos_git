...
class Item extends CI_Model
{
	...
	*/
	...
	/*V1.1*/
	public function get_search_suggestions($search="", $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE,"category"=>""), $unique = FALSE, $limit = 25, $input_search = FALSE)
	{
		$suggestions = array();
		$non_kit = array(ITEM, ITEM_AMOUNT_ENTRY);

		$this->db->select($this->get_search_suggestion_format('item_id, name, pack_name,pic_filename'));/*V1.1*/
		$this->db->from('items');
		$this->db->where('deleted', $filters['is_deleted']);
		$this->db->where('show_cart', 1);
	...
	//		$this->db->where('category', $filters['category']);
		foreach($this->db->get()->result() as $row)
		{
			/*V1.1*/
			if($row->pic_filename){
				$suggestions[] = array('value' => $row->item_id, 'label' => $this->get_search_suggestion_label($row),'icon' =>site_url('items/pic_thumb/' .$row->pic_filename));
			}else{
				$suggestions[] = array('value' => $row->item_id, 'label' => $this->get_search_suggestion_label($row),'icon' =>site_url('items/pic_thumb/no-img.png'));	
			}
			/*V1.1*/
