...
class Item_kits extends Secure_Controller
{
	...
	*/
	...
	public function view($item_kit_id = -1)
	{
		...
		$data['logo_exists'] = $info->kit_filename != '';
		$ext = pathinfo($info->kit_filename, PATHINFO_EXTENSION);
		if($ext == '')
		{
			// if file extension is not found guess it (legacy)
			$images = glob('uploads/item_pics/' . $info->kit_filename . '.*');
		}
		else
		{
	...
	{
		$suggestions = $this->xss_clean($this->Item_kit->get_category_suggestions($this->input->get('term')));
	...
	public function save($item_kit_id = -1)
	{
		...
		if(!empty($upload_data['orig_name']))
		{
			...
			{
				$item_kit_data['kit_filename'] = $upload_data['raw_name'].$upload_data['file_ext'];
			}
		}
