...
/*POS*/	public function pos(){
	...
	public function search()
	{
		...
		$parent_data = $this->Sale->get_parent_data($data_rows);
		echo json_encode(array('total' => $total_rows, 'rows' => $data_rows, 'payment_summary' => $payment_summary, 'parent_data' => $parent_data));
	}
	...
	public function edit_item($item_id)
	{
		...
		if($this->form_validation->run() != FALSE)
		{
			$this->sale_lib->edit_item($item_id, $description, $serialnumber, $quantity, $discount, $discount_type, $price, $discounted_total, $kit_temp);
		}
		else
		{
...
/*POS*/		$action =$this->router->fetch_method();
		if($action=="pos" || isset($_GET['action'])){
...
/*pic*/
	...
	public function get_cat(){
		$result1 = $this->Item->get_categories()->result();
		$result2 = $this->Item_kit->get_categories()->result();
	...
	public function get_item_by_cat(){
		...
		$result_item = $this->Item->get_search_suggestions("",['is_deleted'=>false,'search_custom'=>false,'category'=>$cat]);
		
		echo json_encode($result_item);
		die;
	}

	public function get_item_kit_by_cat(){
		$cat=$_GET['cat'];

		$result_kit = $this->Item_kit->get_category_search_suggestions($cat);
		
		echo json_encode($result_kit);
		die;
	}
/*V1.1*/
}
