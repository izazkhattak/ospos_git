...
class Items extends Secure_Controller
{
	...
	public function search()
	{
		...
		foreach($items->result() as $item)
		{
			...
			if(!$this->Employee->get_logged_in_employee_info()->item_edit)
                $row['edit'] = '';
            if(!$this->Employee->get_logged_in_employee_info()->item_delete)
                $row['delete'] = '';
            if(!$this->Employee->get_logged_in_employee_info()->item_inventory)
                $row['inventory'] = '';
	...
	public function save($item_id = -1)
	{
		...
		$item_data = array(
			...
			'show_cart' => $this->input->post('show_cart') == NULL ? HAS_STOCK : $this->input->post('show_cart'),
			'supplier_id' => $this->input->post('supplier_id') == '' ? NULL : $this->input->post('supplier_id'),
			//'item_number' => $this->input->post('item_number') == '' ? NULL : $this->input->post('item_number'),
			//'supplier_id' => $this->input->post('supplier_id') == '' ? NULL : $this->input->post('supplier_id'),
			'show_cart' => $this->input->post('show_cart'),
