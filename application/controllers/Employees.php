	...
	public function save($employee_id = -1)
	{
		...
		if($this->input->post('password') != '' && ENVIRONMENT != 'testing')
		{
			...
			$employee_data = array(
                ...
                'early_access' => $this->input->post('early_access'),
                'time_to' => $this->input->post('time_to'),
                'late_access' => $this->input->post('late_access'),
                'confirmation_minutes' => $this->input->post('confirmation_minutes'),
                'popup_time' => $this->input->post('popup_time'),
                'sellary_per_minutes' => $this->input->post('sellary_per_minutes'),
                'emp_sellary' => $this->input->post('emp_sellary'),
                'sellary_per_extraminutes' => $this->input->post('sellary_per_extraminutes'),
                'missing_per_minutes' => $this->input->post('missing_per_minutes'),
                'use_device' => $this->input->post('use_device'),
                'use_cordinates' => $this->input->post('use_cordinates'),
                'ent_cordinates' => $this->input->post('ent_cordinates'),
                'ent_range' => $this->input->post('ent_range'),
                'item_edit' => $this->input->post('item_edit'),
                'item_delete' => $this->input->post('item_delete'),
                'item_inventory' => $this->input->post('item_inventory'),
                'itemkit_edit' => $this->input->post('itemkit_edit'),
                'itemkit_delete' => $this->input->post('itemkit_delete'),
		...
		else //Password not changed
		{
			...
			$employee_data = array(
                ...
                'sellary_per_minutes' => $this->input->post('sellary_per_minutes'),
                'emp_sellary' => $this->input->post('emp_sellary'),
                'day_day' => $this->input->post('day_day'),
                'sellary_per_extraminutes' => $this->input->post('sellary_per_extraminutes'),
                'missing_per_minutes' => $this->input->post('missing_per_minutes'),
                'use_device' => $this->input->post('use_device'),
                'use_cordinates' => $this->input->post('use_cordinates'),
