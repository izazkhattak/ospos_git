...
class Sale_lib
{
		...
		$kit_name = NULL, $kit_default_quantity = 0, $kit_temp = 0, $total_kit_price = 0, $total_kit_original_value = 0, $not_repeated_discount = 0, $kit_id = 0, $mode = 'NULL')
	{
		...
		foreach($items as $item)
		{
			...
			if ($item['kit_name'] == $kit_name && $item['item_id'] == $item_id && $item['item_location'] == $item_location)	
			{
		...
		if(!$itemalreadyinsale || $item_info->is_serialized)
		{
			$item = array($insertkey => array(
					...
					'kit_name' => $kit_name,
					'kit_id' => $kit_id,
                    'kit_default_quantity' => $kit_default_quantity,
					'kit_temp' => $kit_temp,
					'kit_total_temp_price' => $total_kit_price,
					'kit_total_temp_original_price' => $total_kit_original_value,
                    'name' => $item_info->name,
			...
			if($line['kit_name'] != "" && $line['kit_id'] > 0){
				...
				foreach ($items as $key=> $row) {
					if($row['kit_id'] == $kit_unique_id){
						...
						{
							$items[$key]['discount_type'] = $discount_type;
						}

						
						if($discounted_total != NULL && $discounted_total != $items[$key]['discounted_total'])
						{
	...
	{
		$items = $this->get_cart();
		if(isset($items[$line]['item_type'])){			
			$item_type = $items[$line]['item_type'];
			$kit_id = $items[$line]['kit_id'];
			if(!empty($items) && $kit_id > 0){
				foreach($items as $key=> $row){
	...
	public function add_item_kit($kit_name, $external_item_kit_id, $item_location, $discount, $discount_type, $kit_price_option, $kit_print_option, &$stock_warning, $kit_temp_quantity = 1, $quantity_if_purchased = 0, $mode = 'NULL')
	{
		...
		{
			$below_discount = $discount; // Prevent multiple discount on each kit items
			if($key2 > 0 && $discount_type == 1){
				$below_discount = 0;
			}
